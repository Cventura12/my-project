...
export default function SchoolCard({ school, documents }: Props) {
  ...
  const deadlineStr = school.financial_aid_deadline
    ? new Date(school.financial_aid_deadline).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : null;
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {deadlineStr && (
              <span className="text-xs text-gray-500">Aid deadline: {deadlineStr}</span>
            )}
          </div>
        </div>
